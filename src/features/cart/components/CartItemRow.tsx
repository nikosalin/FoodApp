...
export function CartItemRow({ id, name, price, quantity, excludedIngredients, selectedExtras = [] }: CartItem) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 border-b border-border py-4 last:border-0">
      <div>
        <h4 className="font-bold text-char">{name}</h4>
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-full bg-secondary text-primary hover:bg-secondary/70"
          aria-label={`Decrease ${name} quantity`}
        >
          <Minus className="h-3.5 w-3.5" />
        </button>

        <span className="w-6 text-center font-semibold text-char">
          {quantity}
        </span>
